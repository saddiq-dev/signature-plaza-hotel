/**
 * StaffUpdateBooking.java   01/08/2021
 * 
...
/**
 * Class to manage Update Booking Information
 */
...
public class StaffUpdateBooking extends JFrame implements ActionListener 
{
    ...
    //---------------------------------------------------------------------------------------------     
    public void StaffUpdateBooking(String[] info) 
    {  
        ...
        tbillToGuestId.setEnabled(true);
        troomNo.setNextFocusableComponent(tbillToGuestId);
        mainBodySubPanel2.add(tbillToGuestId);
        
       //----------------------------------------------------------------------------------------------------------------------
        //View Booking Sub Panel 2: The hotelStaffId label on the form
        ...
        //View Booking Sub Panel 2: The hotelStaffId textbox on the form
        //Query to retrieve the Guest data from the database and populate the Vector object
        try 
        {
            String id = "";
        ...
        tcheckOutDate.setEditable(true);
        ...
        tbookingStatus.setEnabled(true);
    ...
    public void actionPerformed(ActionEvent e) 
    {
        if (e.getSource() == submit)
        {
            ...
            try 
            {
                String query = "UPDATE BOOKING SET dateBooked = ?, checkInDate = ?, checkOutDate = ?, roomType = ?, RoomNo = ?, BookingStatus = ?, guestId = ?, billToGuestId = ?, hotelStaffId = ?, Paid = ? WHERE bookingId = ?";
                
                da.ps = da.con.prepareStatement(query);
                da.ps.setString(1, tdateBooked.getText());
                da.ps.setString(2, tcheckInDate.getText());
                da.ps.setString(3, tcheckOutDate.getText());
                da.ps.setString(4, troomType.getSelectedItem().toString());
                da.ps.setString(5, troomNo.getSelectedItem().toString().substring(0,1));
                da.ps.setString(6, tbookingStatus.getSelectedItem().toString());
                da.ps.setString(7, tguestId.getSelectedItem().toString().substring(0,1));
                da.ps.setString(8, tbillToGuestId.getSelectedItem().toString().substring(0,1));
                da.ps.setString(9, thotelStaffId.getSelectedItem().toString().substring(0,1));
                String upaid = "";
                if(tpaid.isSelected())
                    upaid = "1";
                else
                    upaid = "0";
                da.ps.setString(10, upaid);
